...
| `id` | YES | **kebab-case**, matches `/^[a-z][a-z0-9-]{0,63}$/` — lowercase letters, digits, hyphens; must start with a letter. Examples: `"word-count"`, `"git-branch"`, `"cursor-position"`. Rejected: `"WordCount"`, `"wordCount"`, `"word_count"`, `"1-foo"`. Unique within this extension; used with `canv.ui.setStatusBarItem`. |
