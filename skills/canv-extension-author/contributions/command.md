...
| `id` | YES | **Dotted-lowercase** (different from other contributions). Matches `/^[a-z][a-z0-9]*(?:\.[a-z][a-z0-9]*)*$/` — lowercase segments separated by dots, no hyphens. Examples: `"format"`, `"wordcount.refresh"`, `"git.pull"`. Rejected: `"format-selection"` (hyphen), `"Format"` (uppercase), `"word_count"` (underscore), `"wordcount."` (trailing dot). Unique within this extension. |
