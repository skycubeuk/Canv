...
## `when` grammar (v1 — single condition only)

The `when` value is exactly one of:
...
The schema (`/^(?:fileExt:\.[a-z0-9]+|isDir|isFile)$/`) **does NOT support boolean operators** — no `&&`, no `||`, no parentheses. To match multiple extensions, declare multiple `menu` contributions:

```json
{ "type": "menu", "menu": "fileTree.context", "command": "format.md", "when": "fileExt:.md" },
{ "type": "menu", "menu": "fileTree.context", "command": "format.md", "when": "fileExt:.markdown" }
```
