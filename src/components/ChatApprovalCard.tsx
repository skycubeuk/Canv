        ...
        <ul className="mt-1 flex max-h-72 flex-col gap-2 overflow-auto">
          {preview.edits.map((e, idx) => (
            <li key={idx} className="rounded-sm border border-default bg-panel/60 p-2">
              ...
              <pre className="mt-1 whitespace-pre-wrap rounded-sm bg-danger-soft px-1.5 py-1 text-[0.85em] line-through">{e.oldText}</pre>
              <pre className="mt-1 whitespace-pre-wrap rounded-sm bg-success-soft px-1.5 py-1 text-[0.85em]">{e.newText}</pre>
            </li>
          ))}
        </ul>
      )}
    ...
    <pre className="mt-1 max-h-48 overflow-auto whitespace-pre-wrap rounded-sm bg-panel/60 p-2 text-[0.85em]">
      {parts.map((p, i) => (
        <span
          key={i}
          className={
            p.added ? 'bg-success-soft'
            : p.removed ? 'bg-danger-soft line-through'
            : ''
          }
        >
          {p.value}
