  ...
  return (
    <div className="my-1">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-2 text-[11px] text-muted hover:text-default px-2 py-0.5 rounded border border-default w-full text-left"
      >
        ...
        {denied ? (
          <span className="ml-1 px-1 rounded bg-amber-700/40 text-amber-200">denied</span>
        ) : error ? (
          <span className="ml-1 px-1 rounded bg-red-800/40 text-red-200">err</span>
        ) : null}
      </button>
      {open && (
        <pre
          data-testid="collapsible-body"
          className="mt-1 ml-4 px-2 py-1 text-[11px] whitespace-pre-wrap break-words border-l border-default text-default"
        >
          {body}
        </pre>
      )}
