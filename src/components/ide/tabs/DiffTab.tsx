...
export function DiffTab({ relPath, baseRef, isActive }: Props) {
  ...
  return (
    <div className="h-full flex flex-col bg-app text-xs font-mono overflow-hidden">
      {/* Toolbar */}
        ...
        {changes.map((change, ci) => {
          ...
          return lines.map((line, li) => (
            <tr key={`${ci}-${li}`} className={bg}>
              <td className="w-6 select-none text-center text-subtle border-r border-default pr-1">
                {marker}
              </td>
