...
export function SuggestionBar({ count, onAcceptAll, onRejectAll }: Props) {
  ...
  return (
    <div className="absolute top-2 right-3 z-10 flex items-center gap-2 rounded-md border border-default bg-panel px-2.5 py-1 text-xs shadow-md">
      <span className="font-medium text-warning-fg">{count} {noun}</span>
      <span className="text-subtle">|</span>
      <button type="button" className="text-success-fg hover:underline" onClick={onAcceptAll}>
        Accept all
      </button>
      <button type="button" className="text-danger-fg hover:underline" onClick={onRejectAll}>
        Reject all
      </button>
    </div>
  )
