...
import { ExtensionsTab, type ExtensionsTabHandle } from './ExtensionsTab'
...
describe('ExtensionsTab', () => {
  ...
  it('installFromFolder ref handle calls pickInstallFolder', async () => {
    function Harness({ onMount }: { onMount: (ref: ExtensionsTabHandle) => void }) {
      const ref = useRef<ExtensionsTabHandle | null>(null)
      useEffect(() => { if (ref.current) onMount(ref.current) })
      return <ExtensionsTab ref={ref} />
    }
    let handle: ExtensionsTabHandle | null = null
    render(<Harness onMount={(h) => { handle = h }} />)
    await waitFor(() => expect(screen.getByText('Alpha')).toBeTruthy())
    expect(handle).not.toBeNull()
    handle!.installFromFolder()
