...
export const readFileTool: Tool<Input, Output> = {
  name: 'read_file',
  description:
    'Read the full text of a workspace file. Caps: ≤10MB, non-binary. ' +
  ...
  async handler(input, ctx) {
    ...
    if (entry.size > MAX_OPEN_BYTES) {
      throw new Error(`File too large (${entry.size} bytes, limit ${MAX_OPEN_BYTES}): ${v.rel}. Try search_workspace.`)
    }
