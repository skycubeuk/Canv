...
describe('chatTotals', () => {
  it('returns zero when no messages', () => {
    expect(chatTotals([], 'anthropic', 'm', {})).toEqual({ tokens: 0, costUsd: 0 })
  })
  ...
  it('sums tokens across messages with usage', () => {
    ...
    expect(chatTotals(msgs, 'anthropic', 'm', {}).tokens).toBe(425)
  })

  it('applies pricing when override is present', () => {
    const msgs = [baseMsg('1', 'assistant', 1_000_000, 500_000)]
    const overrides: Record<string, ModelPricing> = { 'anthropic/claude-x': { input: 3, output: 15 } }
    const result = chatTotals(msgs, 'anthropic', 'claude-x', overrides, {})
  ...
  it('returns 0 cost when no pricing override matches', () => {
    ...
    expect(chatTotals(msgs, 'anthropic', 'unknown', {}, {}).costUsd).toBe(0)
  })
  ...
  it('only counts assistant messages for cost, not user messages', () => {
    ...
    const overrides: Record<string, ModelPricing> = { 'anthropic/m': { input: 3, output: 15 } }
    const result = chatTotals(msgs, 'anthropic', 'm', overrides, {})
