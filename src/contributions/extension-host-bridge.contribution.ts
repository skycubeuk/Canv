        ...
        if (method === 'ai.ask') {
          ...
          void (async () => {
            try {
              ...
              if (configuredProviders(s).length === 0) {
                throw new Error('no API key configured — open Settings and add one')
              }
              // Same effective-default resolution the chat panel uses. Falling
              // back to s.provider here would error any time the stored default
              // differs from what the user actually has keys for.
              const { provider, model } = pickDefaultProviderModel(s)
              const apiKey = s.apiKeys[provider]
              const ollamaConfigured = provider === 'ollama' && !!s.baseUrls?.ollama
