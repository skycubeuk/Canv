...
export function Contributions() {
  ...
  // eslint-disable-next-line react-hooks/refs -- intentional latest-value ref; see component header comment for why we can't put this in useEffect (would lag a render and contributions would see stale services)
  ref.current = services

  const stableServices = useMemo(
    () =>
      // eslint-disable-next-line react-hooks/refs -- Proxy getters/has/ownKeys are invoked at call time (when a contribution reads through stableServices), not during render; ref reads here are by design
