...
describe('fs service IPC handlers', () => {
  ...
  describe('canvConfig:factoryReset', () => {
    ...
    // Regression: without this, the renderer reloads with no localStorage
    // pointer to a workspace but main still holds the previous WORKSPACE in
    // memory. Sites tab queries the old workspace's .canv/site_index.yaml
    // and renders pre-reset entries.
    it('happy: closes in-memory workspace so sites/extensions queries see no workspace after reset', async () => {
      const userData = await fsp.mkdtemp(path.join(os.tmpdir(), 'fs-userdata-'))
      try {
        vi.spyOn(electron.app, 'getPath').mockReturnValue(userData)
        const closeWorkspace = vi.fn(async () => {})
        const onWorkspaceChangedGlobal = vi.fn()
        const localIpc = makeIpcMain()
        svc.registerIpcHandlers(localIpc, baseDeps(root, { closeWorkspace, onWorkspaceChangedGlobal }))
        await localIpc.invoke('canvConfig:factoryReset')
        expect(closeWorkspace).toHaveBeenCalledTimes(1)
        expect(onWorkspaceChangedGlobal).toHaveBeenCalledTimes(1)
      } finally {
        await fsp.rm(userData, { recursive: true, force: true })
      }
    })
